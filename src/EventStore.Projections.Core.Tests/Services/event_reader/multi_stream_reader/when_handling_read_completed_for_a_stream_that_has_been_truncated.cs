...
{
    [TestFixture]
    public class when_handling_read_completed_for_a_stream_that_has_been_truncated : TestFixtureWithExistingEvents
    {
        ...
        public new void When()
        {
                ...
                correlationId, "a", 1, 100, ReadStreamResult.Success,
                    new[]
                        ...
                        new EventRecord(
                            ...
                            ResolvedEvent.ForUnresolvedEvent(
                        new EventRecord(
                            2, 200, Guid.NewGuid(), _secondEventId, 200, 0, "a", ExpectedVersion.Any, DateTime.UtcNow,
                            PrepareFlags.SingleWrite | PrepareFlags.TransactionBegin | PrepareFlags.TransactionEnd,
            ...
            _edp.Handle(
                new ClientMessage.ReadStreamEventsForwardCompleted(
                    correlationId, "b", 0, 10, ReadStreamResult.Success, new[]
                           {
                            ResolvedEvent.ForUnresolvedEvent(
                                new EventRecord(
                                    10, 100, Guid.NewGuid(), _thirdEventId, 100, 0, "b", ExpectedVersion.Any, DateTime.UtcNow,
                                    PrepareFlags.SingleWrite | PrepareFlags.TransactionBegin | PrepareFlags.TransactionEnd,
                                    "event_type1", new byte[] {1}, new byte[] {2})),
                            ResolvedEvent.ForUnresolvedEvent(
                                new EventRecord(
                                    11, 150, Guid.NewGuid(), Guid.NewGuid(), 150, 0, "b", ExpectedVersion.Any, DateTime.UtcNow,
                                    PrepareFlags.SingleWrite | PrepareFlags.TransactionBegin | PrepareFlags.TransactionEnd,
                                    "event_type2", new byte[] {3}, new byte[] {4}))
                        }, new StreamMetadata(truncateBefore: 10), false, "",
                    12, ExpectedVersion.NoStream, false, 200));
        }

        [Test]
        public void should_publish_the_correct_events()
        {
