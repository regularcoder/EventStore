...
namespace EventStore.Core.Index
{
    ...
    public partial class PTable : ISearchTable, IDisposable
    {
        ...
        private bool _useMemoryMappedFiles = false;
        private bool _skipIndexVerification = false;

        internal Midpoint[] GetMidPoints() {return _midpoints;} 

        private PTable(string filename,
                       Guid id,
                       bool useMemoryMappedFiles,
                       ...
                       int depth = 16)
        {
            ...
            try
            {
                if (_useMemoryMappedFiles && !_skipIndexVerification) { 
                    VerifyFileHash();
                }
                else
                {
