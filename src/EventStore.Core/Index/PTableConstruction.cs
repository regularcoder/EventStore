...
namespace EventStore.Core.Index
{
    ...
    {
        public static PTable FromFile(string filename, bool useMemoryMappedFiles, bool skipIndexVerification, int cacheDepth)
        {
            return new PTable(filename, Guid.NewGuid(), useMemoryMappedFiles, skipIndexVerification, depth: cacheDepth);
        }

        public static PTable FromMemtable(IMemTable table, string filename, bool useMemoryMappedFiles = false, bool skipIndexVerification = false, int cacheDepth = 16)
        {
            ...
            return new PTable(filename, table.Id, useMemoryMappedFiles, skipIndexVerification, depth: cacheDepth);
        }

        public static PTable MergeTo(IList<PTable> tables, string outputFile, Func<string, ulong, ulong> upgradeHash, Func<IndexEntry, bool> existsAt, Func<IndexEntry, Tuple<string, bool>> readRecord, byte version, bool useMemoryMappedFiles = false, bool skipIndexVerification = false, int cacheDepth = 16)
        {
            ...
            if (tables.Count == 2)
                return MergeTo2(tables, fileSize, indexEntrySize, outputFile, upgradeHash, existsAt, readRecord, version, useMemoryMappedFiles, skipIndexVerification, cacheDepth); // special case
            ...
            return new PTable(outputFile, Guid.NewGuid(), useMemoryMappedFiles, skipIndexVerification, depth: cacheDepth);
        }
        ...
        private static PTable MergeTo2(IList<PTable> tables, long fileSize, int indexEntrySize, string outputFile,
                                       ...
                                       byte version, bool useMemoryMappedFiles, bool skipIndexVerification, int cacheDepth)
        {
