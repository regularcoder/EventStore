...
namespace EventStore.Core.Index
{
    public class IndexMap
    {
        ...
        public static IndexMap FromFile(string filename, int maxTablesPerLevel = 4, bool loadPTables = true, bool useMemoryMappedFiles = false, bool skipIndexVerification = false, int cacheDepth = 16)
        {
            ...
            using (var f = File.OpenRead(filename))
            {
                ...
                using (var reader = new StreamReader(f))
                {
                    ...
                    var tables = loadPTables ? LoadPTables(reader, filename, checkpoints, useMemoryMappedFiles, skipIndexVerification, cacheDepth) : new List<List<PTable>>();
        ...
        private static List<List<PTable>> LoadPTables(StreamReader reader, string indexmapFilename, TFPos checkpoints, bool useMemoryMappedFiles, bool skipIndexVerification, int cacheDepth)
        {
            ...
            while ((text = reader.ReadLine()) != null)
            {
                ...
                try
                {
                    ...
                    ptable = PTable.FromFile(ptablePath, useMemoryMappedFiles, skipIndexVerification, cacheDepth);
                                     ...
                                     int indexCacheDepth = 16)
        {
            ...
            for (int level = 0; level < tables.Count; level++)
            {
                if (tables[level].Count >= _maxTablesPerLevel)
                {
                    ...
                    PTable table = PTable.MergeTo(tables[level], filename, upgradeHash, existsAt, recordExistsAt, version, useMemoryMappedFiles, skipIndexVerification, indexCacheDepth);
